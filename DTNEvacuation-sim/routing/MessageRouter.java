...
public abstract class MessageRouter {
    ...
    public void init(DTNHost host, List<MessageListener> mListeners) {
        ...
        this.evacuationMode = settings.valueFillString(settings.getSetting(EVACUATION_MODE_S));
                ...
                if (nrofHops == 0) {
                    if (from.getGroupId().equals("P")) {
                        for (Map.Entry<Integer,List<Coord>> entry : m.getEvacuationRouteList().entrySet()) {
                            ...
                            evacuationRouteList.put(from.getAddress(),tileList);
                        }
                        m.setEvacuationRouteList(evacuationRouteList);
                    }
                } else {
                    if (getHost().getGroupId().equals("P")) {
                        List<DTNHost> path = m.getHops();
                        DTNHost fromHost = path.get(nrofHops-1);
                        Integer fromAddress = fromHost.getAddress();
                        int tileSizeX = getHost().getTileSizeX();
                        int tileSizeY = getHost().getTileSizeY();
                        int tileX = (int)getHost().getLocation().getX() / tileSizeX;
                        int tileY = (int)getHost().getLocation().getY() / tileSizeY;
                        MovementModel movement = getHost().getMovementModel();
                        int maxTileX = (movement.getMaxX() / tileSizeX) - 1;
                        int maxTileY = (movement.getMaxY() / tileSizeY) - 1;
                        if (tileX > maxTileX) {
                            tileX -= 1;
                        }
                        if (tileY > maxTileY) {
                        ...
                        for (Map.Entry<Integer,List<Coord>> entry : m.getEvacuationRouteList().entrySet()) {
                            if (fromAddress != m.getTo().getAddress()) {
                                List<Coord> tileList = new ArrayList<Coord>(entry.getValue());
                                tileList.add(currentTile);
                                evacuationRouteList.put(getHost().getAddress(),tileList);
                            }
                        }
                    } else if (getHost().getGroupId().equals("AP")) {
                        DTNHost apHost = getHost();
                        int apAddress = apHost.getAddress();
                        List<Coord> tileList = new ArrayList<Coord>(apHost.getEvacuationRouteList().get(apAddress));
                        evacuationRouteList.put(apAddress,tileList);
                    } else if (getHost().getGroupId().equals("CM")) {
